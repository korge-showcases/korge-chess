    ...
    override suspend fun SContainer.sceneMain() {
        uiVerticalStack(adjustSize = false) {
            uiButton("START GAME!").onClick {
                launch { sceneContainer.changeTo { GameScene() } }
            }
...
class GameScene : PixelatedScene(128 * 8, 128 * 8, sceneSmoothing = true) {
    ...
    override suspend fun SContainer.sceneMain() {
        ...
        uiHorizontalStack {
            uiButton("QUIT GAME").onClickSuspend {
    ...
    fun redrawBoard() {
        ...
        board.each { piecePos, v ->
            val image = piecesContainer.image(v.getImage()).centered.xy(piecePos.toDouble() * Size(128, 128) + Point(64, 64))
                .scale(0.9)
                .alpha(
                    when (currentPlayer) {
                        null -> 1f
                        ...
                        if (it.start) {
                            ...
                            for (movement in validMovements) {
                                ...
                                placeholderContainer.solidRect(128, 128, color.withAd(0.25))
                                    .xy(movement.toDouble() * Size(128, 128))
                            }
                        }
                        if (it.end) {
                            if (isValidMovement) {
